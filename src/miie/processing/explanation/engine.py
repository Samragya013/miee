...
class ExplanationEngine(IExplanationEngine):
    ...
    def generate(self, evidence_package: EvidencePackage,
        ...
        if integrity_overall >= 0.8:
            narratives.append(f"Integrity score is high ({integrity_overall:.2f}), indicating strong data consistency and reliability.")
        elif integrity_overall >= 0.5:
            narratives.append(f"Integrity score is moderate ({integrity_overall:.2f}), suggesting acceptable data quality with some inconsistencies.")
        else:
            narratives.append(f"Integrity score is low ({integrity_overall:.2f}), indicating potential data quality issues or anomalies.")
        ...
        if confidence_overall >= 0.8:
            narratives.append(f"Confidence score is high ({confidence_overall:.2f}), indicating reliable assessment based on sufficient data quality and coverage.")
        elif confidence_overall >= 0.5:
            narratives.append(f"Confidence score is moderate ({confidence_overall:.2f}), suggesting reasonable assessment but could benefit from more data.")
        else:
        ...
        if hasattr(evidence_package, 'detector_outputs') and evidence_package.detector_outputs:
            ...
            if isinstance(detector_outputs, dict):
                ...
                detector_list = list(detector_outputs.detector_outputs.keys()) if hasattr(detector_outputs, 'detector_outputs') else []
            detector_count = len(detector_list)
            narratives.append(f"Results based on outputs from {detector_count} detectors: {', '.join(detector_list[:3])}{'...' if detector_count > 3 else ''}.")
        ...
        if detector_filter and hasattr(evidence_package, 'detector_outputs'):
            ...
            if detector_filter in det_dict:
                ...
                narratives.append(f"Detector {detector_filter} output: {list(det_output.keys())[:5]}{'...' if len(det_output) > 5 else ''}.")
