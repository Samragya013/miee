...
Implements the Energy Distance for distribution drift detection with
...
- Consistent against all distributional alternatives
...
def _compute_energy_distance(s1: np.ndarray, s2: np.ndarray) -> float:
    """
    ...
    # Cross-distances
    cross_dist = np.mean(np.abs(np.subtract.outer(s1, s2)))

    # Within-distances
    within_s1 = np.mean(np.abs(np.subtract.outer(s1, s1)))
    within_s2 = np.mean(np.abs(np.subtract.outer(s2, s2)))

    return 2 * cross_dist - within_s1 - within_s2
...
def energy_distance_test(
    ...
    """
    Two-sample test using Energy Distance with permutation p-value.

    This test determines whether two samples come from the same distribution
    by comparing the observed energy distance to its null distribution
    ...
    Returns:
        Tuple of (energy_distance, p_value)
        - energy_distance: Observed energy distance between samples
    ...
    Example:
        ...
        >>> dist, p = energy_distance_test(w1, w2)
        >>> print(f"Energy Distance: {dist:.4f}, p-value: {p:.4f}")

        >>> # Two windows from different distributions
        >>> w3 = np.random.normal(60, 10, 30)  # Shifted mean
        >>> dist, p = energy_distance_test(w1, w3)
        >>> print(f"Energy Distance: {dist:.4f}, p-value: {p:.4f}")
    """
    # Validate inputs
    ...
    observed_distance = _compute_energy_distance(sample1, sample2)
    ...
    for i in range(n_permutations):
        ...
        perm_distances[i] = _compute_energy_distance(perm[:n1], perm[n1:])
...
def energy_distance_drift_detector(
    ...
    for i, window in enumerate(windows[1:], start=1):
        dist, p_value = energy_distance_test(reference_window, window, n_permutations, random_state)

        drift_detected = p_value < alpha

        window_result = {"window_idx": i, "energy_distance": dist, "p_value": p_value, "drift_detected": drift_detected}
...
if __name__ == "__main__":
    ...
    w2 = np.random.normal(50, 10, 30)
    dist, p = energy_distance_test(w1, w2)
    print(f"Same distribution: Energy Distance={dist:.4f}, p={p:.4f}")

    # Different distributions
    w3 = np.random.normal(60, 10, 30)
    dist, p = energy_distance_test(w1, w3)
    print(f"Different distributions: Energy Distance={dist:.4f}, p={p:.4f}")
