...
class IReportGenerator(Protocol):
    ...
    def generate(self, analysis_result: Dict[str, Any],
        ...
        Args:
            ...
            output_formats: List of formats ("json", "md", "csv")
            output_dir: Directory to write output files

        Returns:
            ReportOutput with file paths and manifest location
...
class IJSONExporter(Protocol):
    """INT-16.1: JSON Export Contract"""

    def export(self, analysis_result: Dict[str, Any]) -> str:
...
@runtime_checkable
class ICsvExporter(Protocol):
    """INT-16.2: CSV Export Contract"""
...
@runtime_checkable
class IMarkdownExporter(Protocol):
    """INT-16.3: Markdown Export Contract"""
    ...
    def export(self, analysis_result: Dict[str, Any]) -> str:
        ...
        Returns:
            Markdown string representation of results
...
@runtime_checkable
class ICLIIngest(Protocol):
    """INT-17.1: miie ingest CLI Contract"""

    def execute(self, repo_path: str, shallow: Optional[int] = None) -> int:
        """Execute miie ingest command.

        Args:
            repo_path: Repository path or URL
            shallow: Depth for shallow clone (optional)

        Returns:
            Exit code (0=success, 2=system error, 3=invalid input)
        """
        ...


@runtime_checkable
class ICLIAnalyze(Protocol):
    """INT-17.2: miie analyze CLI Contract"""

    def execute(self, repo_path: str, since: Optional[str] = None,
                ...
                format: Optional[List[str]] = None, exclude_bots: bool = False) -> int:
        """Execute miie analyze command.

        Args:
            repo_path: Repository path or URL
            since: Start datetime (ISO 8601 string)
            until: End datetime (ISO 8601 string)
            metrics: List of metric IDs or ["all"]
            window_strategy: Segmentation strategy
            window_size: Size for segmentation
            output_dir: Output directory path
            detectors: List of detector IDs or ["all"]
            format: Output formats ("json", "md", "csv")
            exclude_bots: Whether to exclude bot commits

        Returns:
            Exit code (0=IS=1.0, 1=IS<1.0, 2=error, 3=invalid input)
        """
        ...


@runtime_checkable
class ICLIDetect(Protocol):
    """INT-17.3: miie detect CLI Contract"""

    def execute(self, metrics_file: str, windows_file: str,
                detectors: Optional[List[str]] = None,
                output_file: Optional[Path] = None) -> int:
        """Execute miie detect command.

        Args:
            metrics_file: Path to pre-extracted metrics JSON
            windows_file: Path to window definitions JSON
            detectors: List of detector IDs or ["all"]
            output_file: Path for detector results output
...
@runtime_checkable
class ICLIBenchmark(Protocol):
    """INT-17.4: miie benchmark CLI Contract"""

    def execute(self, suite: str, detectors: Optional[List[str]] = None,
                seed: Optional[int] = None, output_dir: Optional[Path] = None) -> int:
        """Execute miie benchmark command.
        ...
        Returns:
            Exit code (0=success, 4=benchmark failure)
        """
        ...


@runtime_checkable
class ICLIEvaluate(Protocol):
    """INT-17.5: miie evaluate CLI Contract"""

    def execute(self, benchmark_run_file: str, ground_truth_file: str,
                output_file: Optional[Path] = None) -> int:
        """Execute miie evaluate command.

        Args:
            benchmark_run_file: Path to benchmark run JSON
            ground_truth_file: Path to ground truth JSON
            output_file: Path for evaluation results output
...
@runtime_checkable
class ICLIExplain(Protocol):
    """INT-17.6: miie explain CLI Contract"""

    def execute(self, input_dir: str, metric: Optional[str] = None,
                detector: Optional[str] = None, format: Optional[str] = None) -> int:
        """Execute miie explain command.

        Args:
            input_dir: Directory containing analysis results
            metric: Optional metric ID to filter explanations
            detector: Optional detector ID to filter explanations
            format: Output format ("md" or "json")

        Returns:
            Exit code (0=success, 3=invalid input)
        """
        ...


@runtime_checkable
class ICLIExport(Protocol):
    """INT-17.7: miie export CLI Contract"""

    def execute(self, input_dir: str, format: str,
                filter: Optional[str] = None) -> int:
        """Execute miie export command.

        Args:
            input_dir: Directory containing analysis results
            format: Output format ("json", "md", "csv")
            filter: Optional filter for output (e.g., "failures_only")
...
@runtime_checkable
class ICLIGenerate(Protocol):
    """INT-17.8: miie generate CLI Contract"""

    def execute(self, count: int, output_dir: Optional[Path] = None,
                seed: Optional[int] = None) -> int:
        """Execute miie generate command.
