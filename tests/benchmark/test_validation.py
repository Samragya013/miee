...
def test_candidate_is_git_repo():
    ...
    for candidate_id, candidate_info in data["candidates"].items():
        ...
        result = subprocess.run(["git", "status"], cwd=candidate_path, capture_output=True, text=True)
...
def test_manifest_paths_match_directories():
    ...
    for candidate_id, candidate_info in data["candidates"].items():
        ...
        if manifest_path.exists():
            ...
            assert (
                manifest_path.parent == datasets_candidates_dir
...
def test_anomaly_flags_consistent():
    ...
    for candidate_id, candidate_info in data["candidates"].items():
        ...
        if anomaly_present:
            ...
            assert (
                anomaly_type is not None or len(anomaly_types) > 0
            ...
            for atype in all_types:
                assert isinstance(atype, str) and len(atype) > 0, f"Invalid anomaly type {atype} for {candidate_id}"
        else:
