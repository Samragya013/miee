...
from miie.schemas.models import (
    D01Output, D02Output, D03Output, DetectorResults,
...
class TestD01Output:
    def test_valid_construction(self):
        out = D01Output(
            ks_statistic=0.12, ks_p_value=0.04, psi_value=0.08,
            direction="mean_shift", severity=0.6, flagged=True,
        )
    ...
    def test_valid_directions(self):
        for d in ("mean_shift", "variance_collapse", "shape_change"):
            out = D01Output(ks_statistic=0.1, ks_p_value=0.05, psi_value=0.1,
                            direction=d, severity=0.5, flagged=False)
            assert out.direction == d

    def test_invalid_direction(self):
        with pytest.raises(ValueError, match="direction"):
            D01Output(ks_statistic=0.1, ks_p_value=0.05, psi_value=0.1,
                      direction="invalid", severity=0.5, flagged=False)

    def test_severity_out_of_range(self):
        with pytest.raises(ValueError, match="severity"):
            D01Output(ks_statistic=0.1, ks_p_value=0.05, psi_value=0.1,
                      direction="mean_shift", severity=1.5, flagged=False)
...
class TestD02Output:
    def test_valid_construction(self):
        out = D02Output(
            pearson_r=0.85, spearman_r=0.82, fisher_z_ci=(0.7, 0.9),
            breakdown_type="sudden_drop", delta_r=-0.3, severity=0.7, flagged=True,
        )
        assert out.breakdown_type == "sudden_drop"

    def test_none_breakdown_type(self):
        out = D02Output(
            pearson_r=0.9, spearman_r=0.88, fisher_z_ci=(0.8, 0.95),
            breakdown_type=None, delta_r=0.0, severity=0.2, flagged=False,
        )
        assert out.breakdown_type is None

    def test_valid_breakdown_types(self):
        for bt in ("sudden_drop", "sign_reversal", "gradual_erosion", "confidence_exclusion"):
            out = D02Output(pearson_r=0.5, spearman_r=0.5, fisher_z_ci=(0.4, 0.6),
                            breakdown_type=bt, delta_r=0.1, severity=0.3, flagged=False)
            assert out.breakdown_type == bt

    def test_invalid_breakdown_type(self):
        with pytest.raises(ValueError, match="breakdown_type"):
            D02Output(pearson_r=0.5, spearman_r=0.5, fisher_z_ci=(0.4, 0.6),
                      breakdown_type="unknown", delta_r=0.1, severity=0.3, flagged=False)
...
class TestD03Output:
    def test_valid_construction(self):
        out = D03Output(
            excess_mass_z=2.5, excess_mass_flag=True, dip_test_p_value=0.01,
            multimodal_flag=True, compression_index=0.4, threshold=1.0,
            severity=0.8, flagged=True,
        )
        assert out.flagged is True
    ...
    def test_severity_out_of_range(self):
        with pytest.raises(ValueError, match="severity"):
            D03Output(excess_mass_z=1.0, excess_mass_flag=False, dip_test_p_value=0.5,
                      multimodal_flag=False, compression_index=0.1, threshold=1.0,
                      severity=2.0, flagged=False)
...
class TestDetectorResults:
    ...
    def test_valid_typed_fields(self):
        d01_out = D01Output(ks_statistic=0.1, ks_p_value=0.05, psi_value=0.1,
                            direction="mean_shift", severity=0.5, flagged=True)
        dr = DetectorResults(
    ...
    def test_full_nested_d03(self):
        d03_out = D03Output(excess_mass_z=1.0, excess_mass_flag=False, dip_test_p_value=0.5,
                            multimodal_flag=False, compression_index=0.1, threshold=1.0,
                            severity=0.3, flagged=False)
        dr = DetectorResults(
            d_03={"M-02": {"0.5": {"w01": d03_out}}}
        )
...
class TestExplanation:
    def test_valid_construction(self):
        exp = Explanation(
            ...
            severity="severe", evidence_refs=["ref_001"],
            confidence="high", rule_fired="drift_threshold",
        )
    ...
    def test_invalid_severity(self):
        with pytest.raises(ValueError, match="severity"):
            Explanation(metric_id="M-01", detector_id="D-01",
                        narrative="x", severity="critical",
                        evidence_refs=[], confidence="high", rule_fired="r")

    def test_invalid_confidence(self):
        with pytest.raises(ValueError, match="confidence"):
            Explanation(metric_id="M-01", detector_id="D-01",
                        narrative="x", severity="mild",
                        evidence_refs=[], confidence="certain", rule_fired="r")
...
class TestExplanationReport:
    ...
    def test_report_with_explanations(self):
        exp = Explanation(
            metric_id="M-01", detector_id="D-01",
            narrative="Test", severity="mild",
            evidence_refs=[], confidence="medium", rule_fired="rule_1",
        )
        report = ExplanationReport(
            explanations=[exp],
...
class TestBenchmarkRun:
    ...
    def test_full_construction(self):
        br = BenchmarkRun(
            run_id="run-001", suite_id="suite-001",
            detector_id="D-01", detector_version="1.0.0",
            seed=42, started_at="2026-01-01T00:00:00Z",
            completed_at="2026-01-01T00:05:00Z",
            predictions={"repo_001": {"M-01": {"w01-w02": True}}},
            timing={"repo_001": 1.23},
            environment={"miie_version": "1.0.0", "python_version": "3.11", "platform": "linux"},
        )
...
class TestEvaluationResult:
    ...
    def test_full_construction(self):
        cm = ConfusionMatrix(tp=80, fp=10, tn=85, fn=5)
        er = EvaluationResult(
            suite_id="suite-001", detector_id="D-01", detector_version="1.0.0",
            metrics={"accuracy": 0.92, "precision": 0.89, "recall": 0.94,
                     "f1": 0.91, "auc_roc": 0.95, "auc_pr": 0.93,
                     "fpr": 0.10, "fnr": 0.06},
            confusion_matrix=cm,
...
class TestGroundTruthLabel:
    def test_valid_construction(self):
        label = GroundTruthLabel(
            repo_id="repo_001", metric_id="M-01", label=True,
            event_type="MDE-01", severity="severe",
            evidence_refs=["ref_001"], annotator_agreement=0.9,
        )
    ...
    def test_valid_all_event_types(self):
        for et in ("MDE-01", "MDE-02", "MDE-03", "MDE-04"):
            label = GroundTruthLabel(repo_id="repo_002", metric_id="M-01",
                                     label=False, event_type=et)
            assert label.event_type == et

    def test_invalid_repo_id(self):
        with pytest.raises(ValueError, match="repo_id"):
            GroundTruthLabel(repo_id="invalid", metric_id="M-01",
                             label=True, event_type="MDE-01")

    def test_invalid_event_type(self):
        with pytest.raises(ValueError, match="event_type"):
            GroundTruthLabel(repo_id="repo_001", metric_id="M-01",
                             label=True, event_type="MDE-05")
    ...
    def test_optional_fields(self):
        label = GroundTruthLabel(
            repo_id="repo_001", metric_id="M-01", label=True,
            event_type="MDE-01",
            window_id="w01", window_pair=["w01", "w02"],
            metric_pair=["M-01", "M-02"], threshold=0.5,
        )
...
class TestGroundTruthInput:
    ...
    def test_full_construction(self):
        lbl = GroundTruthLabel(repo_id="repo_001", metric_id="M-01",
                               label=True, event_type="MDE-01")
        gt = GroundTruthInput(
            suite_id="suite-001", version="1.0.0",
            labels=[lbl],
        )
...
class TestDetectorResultBackwardCompat:
    ...
    def test_new_typed_fields(self):
        d01 = D01Output(ks_statistic=0.1, ks_p_value=0.05, psi_value=0.1,
                        direction="mean_shift", severity=0.5, flagged=True)
